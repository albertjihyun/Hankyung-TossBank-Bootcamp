...
한국경제신문×토스뱅크 부트캠프 최종 프로젝트(2026.06~08). 풀스택 트랙 4인 팀이 LLM 트랙과
합동으로 개발했으며, **이 저장소(백엔드)가 본인 담당**이다. 개발 당시 커밋 히스토리는 팀
조직 저장소([toss-delta-final/jarvis-backend](https://github.com/toss-delta-final/jarvis-backend))에
있고, 여기는 그 보존본이다. 서비스는 자비스(JARVIS)로 시작해 **나비스(Narvis)** 로 개명,
`narvis.shop`으로 배포·시연 후 2026-08 운영을 종료했다.
...
| 프론트엔드 ([jarvis-frontend](../jarvis-frontend/)) | 사용자·판매자 화면, 채팅 UI, 실시간 스트림 소비 |
| AI 서버 ([jarvis-ai](../jarvis-ai/)) | 대화형 추천·장바구니 에이전트, 판매자 분석 챗봇, 개인화 프로필 |

**설계의 중심은 "AI가 편해져도 데이터의 주인은 서버 하나"** 다. AI가 실행하는 쓰기(담기·상품
수정·발송 처리)는 전부 내부 연동 API를 지나고, 그 안에서 권한·재고·상태 전이가 사용자
요청과 **똑같이** 검증된다. AI 서버에는 커머스 DB 접근 권한을 주지 않는다 — 읽기 전용도
([ADR-0002](../docs/adr/0002-llm-no-direct-db-access.md)).

### 핵심 기능

- **인증·권한** — 액세스/리프레시 토큰을 HttpOnly 쿠키로, 로그아웃 즉시 기존 토큰 무효화, 게스트 활동의 회원 승계
- **커머스 트랜잭션** — 주문 시점 스냅샷, 옵션 단위 재고 차감, 상태 전이 검증, 취소·반품 자동 승인
- **AI 연동 관문** — 공개 API 45종 + 내부 연동 API 29종, 서비스 토큰 인증 + 입력 전건 재검증
...
- 애플리케이션은 상태를 갖지 않아 4대가 동일하게 동작한다. 로그인 상태는 토큰과 Redis에 있다 — 그래서 1대에서 4대로 늘릴 때 코드 변경이 0이었다.
- **채팅 스트리밍만 예외적으로** 프론트엔드가 AI 서버에 직접 연결한다. 백엔드는 단명 RS256 티켓을 발급하고, AI 서버는 공개키(JWKS)로 검증만 한다 ([ADR-0005](../docs/adr/0005-sse-direct-connect.md)).
...
| 데이터베이스 | **MariaDB 11.4** | 생성 컬럼 + UNIQUE로 "기본 배송지 1개" 같은 제약을 앱이 아닌 DB가 강제 ([ADR-0006](../docs/adr/0006-mysql-to-mariadb.md)) |
| 캐시·공유 상태 | **Redis** | 서버 4대가 공유해야 하는 것만 담는다 — 캐시·채팅 세션·분산 락·요청 횟수 제한 |
| 이벤트 스트림 | **Kafka (KRaft)** | 행동 이벤트 적재를 요청 처리에서 떼어내 응답 지연을 없애고, 같은 스트림을 실시간 집계가 함께 구독 ([ADR-0012](../docs/adr/0012-kafka-single-use-case.md)) |
| 인증 | **JWT + HttpOnly 쿠키** | 스크립트가 토큰을 읽을 수 없어 유출 경로가 막히고, 서버 렌더링 첫 진입에서도 로그인 상태가 유지된다 ([ADR-0010](../docs/adr/0010-access-token-httponly-cookie.md)) |
...
1. **재고가 사는 곳은 한 군데** — 재고를 상품 컬럼이 아니라 옵션 단위 테이블로 분리하고, 차감은 결제 트랜잭션 안의 조건부 갱신으로만 한다. 담기 시점 검증은 사용자 편의를 위한 사전 안내일 뿐이고 최종 방어선은 결제 시점 하나다. ([ADR-0008](../docs/adr/0008-stock-modeling-reversals.md) — 두 번의 번복 과정 포함)
2. **주문은 스냅샷** — 상품명·가격·배송지를 주문 시점 값으로 복사해 저장한다. 이후 상품 가격이 바뀌거나 배송지가 지워져도 주문 내역이 변하지 않는다.
3. **로그는 고쳐 쓰지 않는다** — 게스트가 회원이 될 때 과거 행동 로그의 주인을 바꾸는 방식을 구현했다가 폐기했다. 공용 PC에서 남의 활동이 귀속될 수 있고 원본 이력이 왜곡되기 때문이다. 지금은 게스트 행에 전환 기록만 남기고 읽는 쪽이 매핑으로 잇는다. ([ADR-0009](../docs/adr/0009-guest-logs-immutable.md))
4. **AI는 죽어도 되는 유일한 계층** — AI 서버 장애 시 추천은 인기 상품으로 대체되고 쇼핑 본체는 정상 동작한다. 모든 외부 호출에 타임아웃을 걸고 실패 경로를 미리 정했다.
5. **실패 격리는 catch가 아니라 트랜잭션 경계** — 대시보드의 부가 집계 하나가 실패하면 그 블록만 비우고 응답하도록 try/catch를 걸었지만, 같은 트랜잭션 안에서는 rollback-only 마킹 때문에 격리가 성립하지 않았다. 집계를 REQUIRES_NEW 별도 빈으로 분리해 "실패해도 나머지는 내려보낸다"를 실제로 성립시켰다. ([트러블슈팅 기록](../docs/troubleshooting.md))
6. **측정하고 고치고 다시 측정한다** — k6 부하 테스트에서 병목이 애플리케이션 CPU → 데이터베이스 CPU → 커넥션 수로 두 번 이동했다. "느린 쿼리"가 아니라 "반복되는 같은 계산"을 기준으로 캐시를 재선정해 처리량 +31%·p95 응답시간 −26%를 실측했다. ([ADR-0011](../docs/adr/0011-cache-expansion-after-measurement.md) · [부하테스트 기록](../docs/load-testing.md))

주요 아키텍처 결정 12건은 [ADR 모음](../docs/adr/)에, 원본 결정 로그는 [docs/backend/](docs/backend/README.md) 스펙 문서 안에 D-번호로 남아 있다.
...
`main` push가 곧 운영 배포라 통합 버퍼로 `dev`를 뒀다.
...
| 문서 | 내용 |
|---|---|
| [../docs/adr/](../docs/adr/README.md) | 아키텍처 결정 기록 12건 (큐레이션) |
| [../docs/troubleshooting.md](../docs/troubleshooting.md) | 트러블슈팅 기록 — MariaDB 예약어 + rollback-only, 업스트림 500 진단 |
| [../docs/load-testing.md](../docs/load-testing.md) | k6 부하테스트 방법론·스크립트·측정 이력 |
