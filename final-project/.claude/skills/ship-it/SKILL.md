---
name: ship-it
description: Finish a unit of work in final-project — run pre-commit checks, then commit, push, and open a PR to main. Use when a feature/fix is done and ready to be committed/pushed/PR'd. Commit/push/PR are pre-authorized; force-push/branch-deletion/main changes need explicit confirmation first.
---

# Ship it (commit → push → PR)

## 1. Pre-commit checklist (run what applies)
- **Frontend changed?** `cd frontend && npm run build` (and lint if configured).
- **Backend changed?** build with explicit `JAVA_HOME` (OpenJDK 21): `./gradlew build`.
