...
| P-2 | GET | /api/products/{id} | 🔓 | 상품 상세: 이미지 목록, 옵션 목록, 정가/판매가, summary/attributes/description, 브랜드 요약, 평점 통계(평균·개수) — 조회 시 PRODUCT_VIEW 이벤트 적재(로그인/게스트 공통) |
...
| S-3 | PATCH | /api/seller/products/{id} | 🏪 | 자사 상품 상세 수정: name, summary, attributes, description, sale_price, status — 본인 브랜드 상품 아니면 403 |
