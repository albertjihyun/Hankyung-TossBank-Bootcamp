...
cart(C-1~4), orders/order_item 스냅샷 생성 + mock 결제(O-1, O-2 — 아이템 PENDING→ORDERED 전이, 01 D9), 주문 조회(O-3, O-4 가능 액션 포함), 배송 전이 + 클레임 자동 승인 스케줄러(01 §6), 클레임 신청/내역(O-5, O-6).
- **완료**: 담기→주문(성공/실패 수단 각각, 실패 주문 아이템이 PENDING으로 남는지 포함)→간격 1분으로 줄인 스케줄러로 DELIVERED 도달→반품 신청→자동 승인→상태·내역 반영까지 e2e 시나리오 통과. 01 문서 §7 체크리스트 전부 확인.
