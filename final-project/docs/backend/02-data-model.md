...
- 재고는 이 대상이 아님 — D8 유지("다 팔린다" 가정). *(2026-07-17: D8은 D33으로 폐기되어 재고 컬럼이 생겼으나, 크롤링 재고를 베이스라인으로 쓸지는 결정된 바 없음 — 초기 재고 값 방침은 D33 OPEN 참조.)*
...
- **트레이드오프**: D8이 피하려던 리스크 부활 — 추천 상품이 품절이면 핵심 데모 플로우(추천→담기→구매)가 막힘 → 시드에서 재고를 넉넉히 초기화해 완화. **OPEN**: 크롤링 1만 개+ 상품의 초기 재고 값 방침 미정 — DEFAULT 0이면 전 상품 구매 불가이므로 시드 파이프라인에서 반드시 채울 것(§5와 함께 LLM 팀 협의).
...
- 재고: 크롤링 상품의 `stock_quantity` 초기값을 시드 파이프라인에서 반드시 채울 것 — DEFAULT 0이면 전 상품 구매 불가 (D33 OPEN, LLM 팀 협의)
