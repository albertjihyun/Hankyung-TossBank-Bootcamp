...
- **cart 헤더 테이블은 두지 않는다** — 장바구니는 회원당 암묵 1개고 자체 속성이 없어, 헤더를 만들면 컬럼이 member_id뿐인 1:1 테이블(조인 비용만 추가)이 된다. "장바구니 = 그 회원의 cart_item 집합". 다중/공유 장바구니가 생기면 cart 헤더 + cart_id 컬럼 추가로 확장. 게스트 장바구니 없음(담기=로그인 필요)은 위 비고와 05 I-2(게스트 담기 403 → 로그인 유도)의 의도된 가입 유도 장치.
