...
- **의도적 제외(고도화 후보)**: 클레임 철회(claim.status 값 추가로 수용), 회원 탈퇴(soft delete 경로), 최근 본 상품 개별 삭제. **마지막 건만 user_event append-only 원칙과 충돌** — 시안에 X 버튼이 있으면 재설계 필요(04 §11 OPEN으로 확인 추적).
